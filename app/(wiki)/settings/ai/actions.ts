...
import { isAiProviderId } from "@/lib/ai/provider";
...
const saveKeyMessages: Record<Extract<SaveApiKeyResult, { ok: false }>["reason"], string> = {
  ...
  malformed: "API 키 형식이 아닙니다. 선택한 공급자의 키인지 확인해주세요.",
  invalid: "공급자가 이 키를 거부했습니다. 키를 다시 확인해주세요.",
  unreachable: "공급자 API에 연결하지 못했습니다. 잠시 후 다시 시도해주세요.",
  unknown: "키를 저장하지 못했습니다. 잠시 후 다시 시도해주세요.",
};

const capSchema = z.coerce.number().int().min(0).max(1_000_000_000);

export async function saveApiKeyAction(provider: unknown, plainKey: string): Promise<AiActionResult> {
  if (!isAiProviderId(provider)) return { ok: false, message: "지원하지 않는 공급자입니다." };
