...
> **보관 문서 — 현재 사실 아님.** 이 문서는 개발 시작 시점의 원본 설계 초안이다. 이후 실제 구현 과정에서 바뀐 내용은 여기 반영되지 않는다. 지금 시점의 사실은 `docs/architecture.md`, `docs/business-rules.md`, `docs/security.md`, `docs/tracking/decisions/`를 따르고, 이 문서와 충돌하면 그쪽이 우선한다. 읽어야 할 문서를 모르겠다면 `docs/INDEX.md`부터 확인한다.
...
LLM 공급자는 `lib/ai/provider.ts`의 `AiProvider` 인터페이스(`verifyKey`, `streamAnswer` → `connected`/`text`/`final`/`abort`, `ProviderError`) 뒤에 둔다. 현재 어댑터는 Anthropic(`@anthropic-ai/sdk`)과 OpenAI(`openai`, Responses API)이며, 라우트·데이터 계층·화면은 공급자를 모른다. **사용자는 설정에서 공급자만 고르고, 모델은 운영자가 배포 설정으로 공급자별로 고정**한다. 소유자 1인 단계에서 모델 선택 UI는 의미가 없고, 모델이 하나면 캐시와 품질 검증 대상이 하나로 모인다. 가입 개방 시 허용 목록 방식으로 확장한다.

공급자를 둘 이상 두는 이유는 결제·지역 제약이다. 실제로 Anthropic Console 크레딧 결제가 카드 문제로 막혔고, OpenAI로 전환해 검증했다. 공급자 추가(예: Gemini)는 어댑터 하나와 `ai_provider` enum 값 추가로 끝나야 한다.

| 항목 | 값 |
|---|---|
| 모델 | `AI_ANTHROPIC_MODEL`(기본 `claude-opus-5`), `AI_OPENAI_MODEL`(기본 `gpt-5-mini`) |
| 클라이언트 | 요청마다 사용자 키로 생성, 전역 인스턴스 금지 |
| 추론 | Anthropic `thinking: adaptive` + `output_config.effort`, OpenAI `reasoning.effort` |
| effort | query `low`(gpt-5-mini에서 `medium`은 추론 토큰 ~1,200·첫 글자 25초), summary 채우기 `low`, 링크 초안 `low`, 모순 검사 `high` — 작업별 서버 상수 |
| 캐시 | 고정 시스템 프롬프트를 앞에 두어 공급자별 프롬프트 캐시가 걸리게 함 |
| 스트리밍 | query는 스트리밍, 요약 채우기는 순차 호출(건수가 늘면 Batches API로 교체) |

### 키 보관

앱 레벨 AES-256-GCM으로 암호화해 `user_ai_settings.encrypted_key`에 저장한다. 키는 서버 환경변수 `AI_KEY_ENCRYPTION_SECRET`에서 SHA-256으로 파생하고, 암호문은 `v1.<iv>.<tag>.<data>`(base64url) 형식이다. Supabase Vault를 쓰지 않은 이유: Vault 복호화 함수를 `authenticated`에게 열면 사용자가 브라우저에서 자기 JWT로 직접 호출해 키를 볼 수 있고, 막으려면 웹 서버에 `service_role`이 필요해 12절 원칙과 충돌한다. 앱 암호화는 RLS로 자기 행의 암호문은 볼 수 있어도 서버 비밀 없이는 쓸모가 없다.

1. 설정 화면에서 공급자 선택·키 입력 → 서버가 공급자별 키 형식을 검사하고 `models.list`로 1회 검증 → 유효할 때만 암호화 저장.
2. 화면에는 공급자와 마지막 4자(`OpenAI 연결됨 · …a1b2`)만 표시한다. 키를 다시 읽는 API는 없고 삭제만 가능하다.
3. 키 삭제 시 `enabled = false`, 암호문·힌트 제거.
4. 로그에 키, 프롬프트 전문, 응답 전문을 남기지 않는다.
5. 비밀 로테이션은 전체 재암호화 작업이 필요하다. 버전 접두어로 두 형식이 공존할 수 있게 해 두었다. 상용화 시 KMS 봉투 암호화로 교체하는 지점은 [상용화-대비-교체-지점.md](./상용화-대비-교체-지점.md)에 있다.
...
|---|---|---|
| `user_id` | uuid PK/FK | 현재 사용자 |
| `enabled` | boolean | 키가 등록·검증된 경우만 `true`(암호문이 있어야 함, check 제약) |
| `provider` | enum | `anthropic`, `openai` |
| `encrypted_key` | text | 앱 암호화 암호문, 선택 |
...
| `id` | uuid PK |  |
| `owner_id` | uuid FK | RLS 기준 |
| `kind` | enum | `query`, `ai_summary`, `draft_page`, `contradiction`, `ingest` |
| `provider` | enum | `anthropic`, `openai` |
| `model` | text | 실제 사용 모델(응답이 보고한 스냅샷 ID) |
| `billing_source` | enum | `byok`(기본), `platform` — 상용화 대비 |
...
소유자는 `select`·`insert`만 가능하다(추가 전용). 추정 비용은 저장하지 않고 화면에서 모델 단가표(`lib/ai/pricing.ts`)로 계산한다(단가는 바뀌므로 기록에 고정하지 않는다).
...
구현은 기존 관례대로 Server Action을 우선하고, 스트리밍이 필요한 곳만 Route Handler를 둔다(`/api/v1` 접두어는 쓰지 않는다).

| 진입점 | 역할 | 주요 정책 |
|---|---|---|
| `saveApiKeyAction(provider, key)` | 키 등록·검증 | 형식 검사 → 공급자 검증 → 암호화 저장 |
| `deleteApiKeyAction()` | 키 삭제 | `enabled = false`, 암호문 제거 |
| `updateMonthlyTokenCapAction(cap)` | 월 토큰 한도 | 0 = 무제한 |
| `POST /api/ai/query` | 카드 내 질문 | `text/plain` 스트리밍, 첫 바이트 전 실패는 JSON 502, 완료 시 `ai_runs` 기록 |
| `createAiDraftDocumentAction` | 답변을 초안 문서로 | `status='draft'` + `frontmatter.ai`, 편집 화면에서 사용자가 저장해야 `active` |
| `generateAiSummariesAction()` | 파생 불가 문서 AI 요약 | 순차 호출, 캡 도달 시 중단, 결과는 `ai_summary` 제안 |
| `applySuggestionAction` / `dismissSuggestionAction` | 제안 승인·기각 | 기존 문서 저장 경로 재사용 |
| (A2) 모순 검사 | | 사전 견적 확인 필수 |
...
| A1 (완료) | 공급자 추상화(Anthropic·OpenAI), BYOK 설정(AES-GCM·검증·마스킹·삭제·월 한도), `ai_runs`와 사용량 화면, 카드 내 Query와 `문서로 저장`, 파생 불가 문서의 AI `summary` 채우기 | 있음 |
