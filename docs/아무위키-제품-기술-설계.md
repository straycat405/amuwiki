...
| `kind` | enum | `fill_summary`, `create_page`, `broken_link`, `orphan`, `forward_link`, `contradiction` |
| `payload` | jsonb | 제안 내용(요약문, 초안 제목·본문, 근거 문서 ID 등) |
| `run_id` | uuid FK | `ai_runs` 참조, SQL 전용 제안은 null |
| `status` | enum | `pending`, `applied`, `dismissed` |
| `created_at` | timestamptz | 필수 |
| `resolved_at` | timestamptz | 승인·기각 시각 |

카파시 패턴의 `lint` 결과 큐다. SQL만으로 만드는 제안(고아·깨진 링크·정방향 링크·빈 요약)과 AI가 만드는 제안이 같은 테이블·같은 승인 UI를 쓴다. 승인은 기존 문서 저장 경로를 그대로 타며 새 쓰기 경로를 만들지 않는다.
...
| 제안 종류 | 생성 방식 | 승인 시 동작 |
|---|---|---|
| 고아 문서(들어오는 링크 0) | SQL | 정보 표시만, 승인 없음 |
| 깨진 Wiki link | SQL | `create_page` 초안 요청으로 전환 가능 |
| 정방향 링크 미해석 | SQL | 해당 문서 재색인 |
| 빈 `summary` | SQL 감지 → AI 생성(Batch) | `summary` 필드만 갱신, 새 리비전 |
| 깨진 링크 초안 | AI | 새 문서 편집 화면으로 이동 |
| 페이지 간 모순 | AI, 사용자가 명시적으로 실행 | 두 문서를 나란히 표시, 수정은 사용자가 직접 |

- SQL 제안은 저장 시점 또는 설정 화면 진입 시 재계산한다.
- `summary` 채우기는 문서당 1회 호출이며 Message Batches API로 보낸다. 실행 전 대상 문서 수와 예상 입력 토큰을 보여준다.
...
| 단계 | 내용 | AI 호출 |
|---|---|---|
| A0 | SQL 제안(고아·깨진 링크·정방향 링크·빈 요약) + `document_suggestions` + 승인 UI | 없음 |
| A1 | BYOK 설정(Vault·검증·마스킹·삭제), `ai_runs`, 사용량 화면, 카드 내 Query와 `문서로 저장`, `summary` 일괄 채우기 | 있음 |
